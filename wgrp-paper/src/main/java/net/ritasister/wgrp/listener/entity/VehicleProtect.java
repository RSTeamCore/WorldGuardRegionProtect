...
public final class VehicleProtect implements Listener {
    ...
    private void denyVehicleCollision(@NotNull VehicleEntityCollisionEvent e) {
        if (!ConfigFields.DENY_COLLISION_WITH_VEHICLE.asBoolean(wgrpPlugin.getWgrpPaperBase())) {
            return;
        }
    ...
    private void denyVehicleEnter(@NotNull VehicleEnterEvent e) {
        if (!ConfigFields.DENY_SIT_AS_PASSENGER_IN_VEHICLE.asBoolean(wgrpPlugin.getWgrpPaperBase())) {
            return;
        }
    ...
    private void denyVehicleDamage(@NotNull VehicleDamageEvent e) {
        if (!ConfigFields.DENY_DAMAGE_VEHICLE.asBoolean(wgrpPlugin.getWgrpPaperBase())) {
            return;
        }
